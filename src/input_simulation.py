...
class InputSimulator:
    ...
    def press_enter(self):
        """
        Simulate pressing the Enter key (e.g. to submit the typed text).
        """
        if self.input_method in ('pynput', 'clipboard'):
            self.keyboard.press(PynputKey.enter)
            self.keyboard.release(PynputKey.enter)
        elif self.input_method == 'ydotool':
            run_command_or_exit_on_failure(["ydotool", "key", "28:1", "28:0"])
        elif self.input_method == 'dotool':
            assert self.dotool_process and self.dotool_process.stdin
            self.dotool_process.stdin.write("key enter\n")
            self.dotool_process.stdin.flush()
