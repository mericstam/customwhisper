...
def _trim_tail(recording, n):
    """Remove up to the last n samples from the recording list (drops a spoken command)."""
    if n > 0 and recording:
        del recording[-min(n, len(recording)):]
...
class ResultThread(QThread):
    ...
    def __init__(self, local_model=None, command_recognizer=None):
        """
        Initialize the ResultThread.

        :param local_model: Local transcription model (if applicable)
        :param command_recognizer: Optional CommandRecognizer for in-dictation voice commands
        """
        super().__init__()
    ...
    def _record_audio(self):
        ...
        # In-dictation voice commands (Vosk): hold / resume / stop / cancel
        cmd = self.command_recognizer
        paused = False
        cancelled = False
        trim_samples = 0
        if cmd:
            cmd.reset()
            # Small safety margin added to the precise (frame-counted) command span.
            margin_ms = ConfigManager.get_config_value('voice_commands', 'trim_ms') or 300
            margin_samples = int(self.sample_rate * (margin_ms / 1000.0))
        ...
        with sd.InputStream(samplerate=self.sample_rate, channels=1, dtype='int16',
            ...
            while self.is_running and self.is_recording:
                ...
                # In-dictation voice command handling (runs every frame)
                if cmd:
                    action, cmd_frames = cmd.process(frame.tobytes())
                    cmd_trim = cmd_frames * frame_size + margin_samples
                    if action == 'hold' and not paused:
                        paused = True
                        _trim_tail(recording, cmd_trim)
                        ConfigManager.console_print('Voice command: HOLD (paused, still listening).')
                        continue
                        ...
                        if vad:
                            silent_frame_count = 0
                        ConfigManager.console_print('Voice command: RESUME.')
                        continue
                    elif action == 'stop':
                        _trim_tail(recording, cmd_trim)
                        ConfigManager.console_print('Voice command: STOP.')
                        break
                    elif action == 'cancel':
                        cancelled = True
                        ConfigManager.console_print('Voice command: CANCEL (discarded).')
                        break

                if paused:
                    # Keep listening for a resume command, but don't record or run VAD.
                    continue
        ...
        if cancelled:
            ConfigManager.console_print('Recording discarded (cancel command).')
            return None
